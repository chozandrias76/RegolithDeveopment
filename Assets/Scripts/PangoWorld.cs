...
public class PangoWorld : MonoBehaviour
{
	...
	public delegate float ToolFunction(Vector3	dlt,float d,float editRadius,float er2,float targDelta);
	
	public static float 	cubeTool(Vector3	dlt,float d,float editRadius,float er2,float targDelta){
		//Cube tool...
		if(fabs(dlt.x)<editRadius&&
	...
	public static float 	sphereTool(Vector3	dlt,float d,float editRadius,float er2,float targDelta){
		//Sphere tool...
	...
	*/
	...
	void	buildRegion(GameObject archetype,ref Region chunk){
	...
	Region getRegion (GameObject archetype, int x, int y, int z)
	{
	...
	void updateGenerator(){
		if(loadingQueue.Count>0){
			...
			buildRegion(gameObject,ref topObj);
		...
		for (int tx=-planarSweep; tx<=planarSweep; tx++)
			for (int ty=-depthSweep; ty<=depthSweep; ty++)
				for (int tz=-planarSweep; tz<=planarSweep; tz++) {
					Region chunk=getRegion (this.gameObject, tx+viewRX, ty+viewRY, tz+viewRZ);
	...
	void getCollidingRegions(Vector3 pmin,Vector3 pmax,ref List<Region>	colliders){
		...
		for(int tz=tmpMin[2];tz<=tmpMax[2];tz++)
			colliders.Add(getRegion(this.gameObject,tx,ty,tz));
	}
...
//		if(mouseCaptured==true&&
			...
			//if (Physics.Raycast(ray, out hit, 500.0f))
			{
				...
				{	
					...
					//if (hit.transform.tag == "Finish")
					{
						...
						editWorld(hit.point,editRadius,editValue,cubeTool);
					}
				}
			}
		}
	...
	void OnGUI () {
		if(simIsPaused==true)
		{
			...
			if (GUI.Button (new Rect (10,115,150,25), "Start!")) {
				pauseSim(false);
				simIsPaused=false;
			}
		}
	...
	void Start ()
	{
		...
		viewerObject.layer=
		cursorObject.layer=LayerMask.NameToLayer("Ignore Raycast");
		//cursorObject.GetComponent<MeshCollider>().enabled=false;
