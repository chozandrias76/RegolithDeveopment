...
{ 
    ...
    { 
      ...
      v2f vert (appdata v) { 
        ...
        float fy=v.texcoord.y;
       	float sinOff=((1.0f-fy)-0.05);
 		float fscl=0.14;
 		float3	wave;
 		
		if(fx<0.50f)
		{
	        v.vertex.x+=(sin(t*1.45*sinOff)-0.5f)*fy*fscl*sinOff; 
	
	       // v.vertex.y+=(sin(t*3.12*sinOff)-0.1f)*fy*fscl-fy*fscl*0.9; 
	
	        v.vertex.z-=(sin(t*2.2*sinOff)-0.5f)*fy*fscl*sinOff; 
		}
