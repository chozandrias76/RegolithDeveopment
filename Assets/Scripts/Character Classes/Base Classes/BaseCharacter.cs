...
//		Debug.Log( "***BaseCharacter - StatUpdate***" );
	...
	public Item EquipedTool {
		...
		set {
			...
			if( _equipment[ (int)EquipmentSlot.MainHand ] != null) {
				GameObject mesh = Instantiate( Resources.Load( GameSetting2.MELEE_ETOOL_MESH_PATH + _equipment[(int)EquipmentSlot.MainHand].Name ), weaponMount.transform.position, weaponMount.transform.rotation ) as GameObject;
				mesh.transform.parent = weaponMount.transform;
			}
		}
	}
