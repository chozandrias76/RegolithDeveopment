...
//	private List<Item> _lootItems;
	...
	*******************************************************/
	...
	public float _editorHorzSliderValue;
...
//		Messenger<int>.AddListener("PopulateChest", PopulateChest);
		...
		Messenger.AddListener("ToggleEditor", ToggleEditorWindow);
	}
...
//		Messenger<int>.RemoveListener("PopulateChest", PopulateChest);
		...
		Messenger.RemoveListener("ToggleEditor", ToggleEditorWindow);
	}
...
//		if(mouseCaptured==true&&
	...
	void OnGUI() {
		...
		if(_displayEditorWindow){
			_editorLocX = (Screen.width/2 - (_editorCols*4*buttonWidth)/2);
			_editorLocY = (Screen.height/2-(_editorRows*buttonHeight)/2);
			_editorScaleX = (_editorCols*buttonWidth*4);
			_editorScaleY = (_editorRows*buttonHeight*2);
			_editorWindowRect = new Rect(_editorLocX, _editorLocY, _editorScaleX, _editorScaleY);
			_editorWindowRect = GUI.Window(EDITOR_WINDOW_ID, _editorWindowRect, EditWindow, "");
		}
...
//								Debug.Log("Reset the double click timer");
	...
	{
		_editorHorzSliderValue = 3.0f;
			if(GUI.Button(new Rect((0 * buttonWidth), (0 * buttonHeight), buttonWidth*4, buttonHeight), "Draw Cube")) 
				{
				editModule = false;
				}
				if(GUI.Button(new Rect((4 * buttonWidth), (0 * buttonHeight), buttonWidth*4, buttonHeight), "Draw Sphere")) 
				{
				editModule = true;
				}
		GUI.HorizontalSlider(new Rect((0 * buttonWidth), (1 * buttonHeight), buttonWidth*8, buttonHeight), _editorHorzSliderValue, _editorHorzSliderMin, _editorHorzSliderMax);
			GUI.DragWindow();
...
/*	void updateTools()
	{
		...
		if(gotHit){
			cursorObject.active=true;
			//renderer.enabled=true;
			//cursorObject.GetComponent<MeshRenderer>().enabled=true;
				...
				{	
					{
						...
						if(editModule){
							editWorld(hit.point,editRadius,editValue,PangoWorld.cubeTool);
						}
						else if(!editModule){
							editWorld(hit.point,editRadius,editValue,PangoWorld.sphereTool);
						}
					}
				}
			}
